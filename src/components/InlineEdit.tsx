      ...
      <div className="flex items-start gap-1.5">
        {multiline ? (
          <textarea
            ...
            rows={3}
            className={`flex-1 bg-white/5 border border-teal-500/50 rounded-lg px-3 py-1.5 text-text-primary text-sm outline-none focus:border-teal-400 resize-y ${inputClassName}`}
          ...
          <input
            ...
            onChange={(e) => setDraft(e.target.value)}
            onKeyDown={handleKeyDown}
            className={`flex-1 bg-white/5 border border-teal-500/50 rounded-lg px-3 py-1.5 text-text-primary text-sm outline-none focus:border-teal-400 ${inputClassName}`}
            placeholder={placeholder}
          />
        )}
    ...
    return (
      <div className="flex items-center gap-1.5">
        ...
        <input
          ...
          max={max}
          step={step}
          className="w-20 bg-white/5 border border-teal-500/50 rounded-lg px-2 py-1 text-text-primary text-sm outline-none focus:border-teal-400"
        />
  ...
  return (
    <div ref={ref} className={`relative inline-block ${className}`}>
      <button
        ...
        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium cursor-pointer hover:ring-1 hover:ring-teal-500/50 transition-all ${current?.color || 'bg-white/10 text-text-muted'}`}
      >
        {current?.label || value}
        ...
        <div className="absolute top-full left-0 mt-1 z-50 bg-surface-2 border border-border rounded-lg shadow-xl py-1 min-w-[120px]">
          {options.map((opt) => (
            <button
              ...
              className={`w-full text-left px-3 py-1.5 text-xs hover:bg-white/10 transition-colors ${opt.value === value ? 'text-teal-400 font-medium' : 'text-text-secondary'}`}
            >
              {opt.label}
            </button>
          ))}
    ...
    <div className="space-y-1.5">
          ...
          {editingIdx === idx ? (
            <div className="flex-1 flex items-center gap-1.5">
              <input
                ...
                className="flex-1 bg-white/5 border border-teal-500/50 rounded px-2 py-1 text-sm text-text-primary outline-none"
                autoFocus
              />
      ...
      {adding ? (
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs text-text-muted hover:text-teal-400 transition-colors mt-1"
        >
...
export function EditBanner({ hasEdits, onReset, label = 'This view has custom edits saved locally' }: EditBannerProps) {
  ...
  return (
    <div className="flex items-center justify-between bg-teal-500/10 border border-teal-500/20 rounded-lg px-4 py-2 mb-4">
      <span className="text-xs text-teal-400">{label}</span>
      <button
        onClick={onReset}
        className="flex items-center gap-1 text-xs text-text-muted hover:text-teal-400 transition-colors"
      >
